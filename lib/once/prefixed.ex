defmodule Once.Prefixed do
  ...
  @bin_formats [:raw | @encoded_formats]
  ...
  defp check_type_opt(params = %{type: :encrypted, get_key: _}), do: params
  defp check_type_opt(%{type: :encrypted}), do: raise(ArgumentError, "you must provide :get_key")

  defp check_type_opt(params = %{encrypt?: true}) do
    Logger.warning("option `:encrypt?` is deprecated, use `type: :encrypted` instead")
    params |> Map.put(:type, :encrypted) |> check_type_opt()
  end

  defp check_type_opt(params), do: params

  defp check_prefix_opt(params = %{prefix: _}) when params.db_format in @bin_formats, do: params
  defp check_prefix_opt(params), do: params

  defp strip_prefix(value, prefix) do
    case value do
      ^prefix <> value -> value
      _ -> :error
    end
  ...
  defp add_prefix({:ok, value}, prefix), do: {:ok, prefix <> value}
  defp add_prefix(_, _), do: :error

  @impl true
  def cast(nil, _), do: {:ok, nil}
  def cast(value, params), do: to_format(value, params.ex_format, params.prefix)

  @impl true
  def load(nil, _, _), do: {:ok, nil}
  def load(value, _, params), do: to_format(value, params.ex_format, params.prefix)

  @impl true
  def dump(nil, _, _), do: {:ok, nil}
  def dump(value, _, params), do: to_format(value, params.db_format, params.prefix)

  @impl true
  def autogenerate(params = %{type: :counter}) do
    NoNoncense.nonce(params.no_noncense, 64) |> to_format!(params.ex_format, params.prefix)
  end

  def autogenerate(params = %{type: :sortable}) do
    NoNoncense.sortable_nonce(params.no_noncense, 64)
    |> to_format!(params.ex_format, params.prefix)
  end
  ...
  def to_format(value, format, prefix) do
    value = strip_prefix(value, prefix)
    value |> identify_format() |> maybe_convert(value, format) |> add_prefix(prefix)
  end
  ...
  def to_format!(value, format, prefix) do
    case to_format(value, format, prefix) do
      {:ok, value} -> value
